    ...
    (async () => {
      ...
      try {
        ...
        if (err instanceof ConvexHttpError) {
          setBackendDown(true);
          setVerifyError(
            "Backend not connected yet — your link couldn't be verified. Try again once the backend is configured.",
          );
        } else {
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-3">
        ...
        {backendDown && (
          <p role="alert" className="text-sm font-medium text-amber-700">
            Backend not connected yet — magic links can&apos;t be sent until the
            backend is configured. Try again later.
          </p>
        )}
        {formError && (
          <p role="alert" className="text-sm font-medium text-red-700">
